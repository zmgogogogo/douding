...
function seedBeads() {
  ...
  const insertSeries = db.prepare(
    'INSERT INTO bead_series (brand_id, name, sort_order) VALUES (?, ?, ?)'
  )
  const insertColor = db.prepare(
    'INSERT INTO bead_colors (series_id, name, hex, sort_order) VALUES (?, ?, ?, ?)'
  )
...
app.post('/api/image-to-grid', authOptional, upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    // 加载珠子颜色并预计算 Lab
    const allColors = db
      .prepare('SELECT c.id, c.name, c.hex FROM bead_colors c ORDER BY c.id')
      .all()
