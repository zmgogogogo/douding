  ...
  try {
    ...
    const txn = db.transaction(() => {
      for (const item of items) {
        if (!item.colorId || item.quantity == null) { failed++; continue }
        const cur = db.prepare('SELECT quantity FROM user_bead_inventory WHERE user_id=? AND color_id=?').get(userId, item.colorId)
        ...
        db.prepare(`INSERT INTO inventory_logs (user_id,color_id,action,quantity,balance_after,source_type,note)
          VALUES (?,'adjust',?,?,'batch',?)`).run(userId, item.colorId, newQty - (cur?.quantity || 0), Math.max(0, newQty), item.note || '')
        success++
      }
    })
    txn()
    res.json({ code: 200, data: { success, failed }, message: `批量操作完成：${success}成功${failed > 0 ? `, ${failed}失败` : ''}` })
  } catch (e) {
    res.status(500).json({ code: 500, message: e.message })
  }
})
  ...
  try {
    ...
    const txn = db.transaction(() => {
      for (const id of colorIds) {
        ...
        db.prepare(`INSERT INTO inventory_logs (user_id,color_id,action,quantity,balance_after,source_type,note)
          VALUES (?,'adjust',0,0,'batch_delete','已删除库存记录')`).run(userId, id)
      }
    })
    txn()
    res.json({ code: 200, data: { deleted: colorIds.length }, message: `已删除 ${colorIds.length} 条库存记录` })
  } catch (e) {
    res.status(500).json({ code: 500, message: e.message })
  }
})
  ...
  try {
    ...
    const txn = db.transaction(() => {
      for (const item of items) {
        ...
        if (diff !== 0) {
          db.prepare('UPDATE user_bead_inventory SET quantity=?, updated_at=datetime(\'now\') WHERE user_id=? AND color_id=?')
            .run(actualQty, userId, item.colorId)
        }
        db.prepare(`INSERT INTO inventory_logs (user_id,color_id,action,quantity,balance_after,source_type,note)
          VALUES (?,'adjust',?,?,'stock_check',?)`)
          .run(userId, diff, actualQty, diff > 0 ? `盘点：盘盈 +${diff}` : diff < 0 ? `盘点：盘亏 ${diff}` : '盘点：一致')
    ...
    const txn = db.transaction(() => {
      for (let i = 0; i < items.length; i++) {
        ...
        if (!colorId && item.name) {
          const found = db.prepare('SELECT id FROM bead_colors WHERE name LIKE ? OR hex=?').get(`%${item.name}%`, item.hex || '')
        ...
        db.prepare(`INSERT INTO user_bead_inventory (user_id,color_id,quantity,min_threshold,unit_cost,location,updated_at)
          VALUES (?,?,?,?,?,?,datetime('now'))
        ...
        db.prepare(`INSERT INTO inventory_logs (user_id,color_id,action,quantity,balance_after,source_type,note)
          VALUES (?,'inbound',?,?,'import',?)`).run(userId, colorId, qty, qty, item.note || '批量导入')
        success++
      }
    })
    txn()
    res.json({ code: 200, data: { success, failed, errors }, message: `导入完成：${success}成功${failed > 0 ? `, ${failed}失败` : ''}` })
  } catch (e) {
    res.status(500).json({ code: 500, message: e.message })
  }
})
  ...
  try {
    ...
    const txn = db.transaction(() => {
      for (const b of beads) {
        ...
        if (curQty < actualQty) warnings.push({ colorId: b.colorId, short: actualQty - curQty, name: b.name || '' })
        db.prepare(`INSERT INTO user_bead_inventory (user_id,color_id,quantity,updated_at) VALUES (?,?,?,datetime('now'))
        ...
        db.prepare(`INSERT INTO inventory_logs (user_id,color_id,action,quantity,balance_after,source_type,source_id,source_name,note)
          VALUES (?,'outbound',?,?,'design',?,?,?)`)
          .run(userId, b.colorId, -actualQty, newQty, designId || null, designTitle || '',
    ...
    res.json({
      code: 200,
      data: { warnings, copies: copyCount, lossRate, totalBeads: beads.reduce((s, b) => s + b.quantity * copyCount, 0) },
      message: warnings.length ? `消耗扣除完成（${warnings.length}种颜色库存不足）` : '消耗扣除成功'
    })
  } catch (e) {
    res.status(500).json({ code: 500, message: e.message })
  }
...
router.post('/inventory/match', authRequired, (req, res) => {
  try {
    ...
    if (designId) {
      // 从图纸数据统计颜色用量
      const design = db.prepare('SELECT grid_data, grid_width, grid_height FROM designs WHERE id=?').get(designId)
    ...
    // 逐色比对
    const sufficient = [], insufficient = [], missing = []
    for (const usage of colorUsage) {
      const stock = stockMap[usage.hex]
      if (stock && stock.quantity >= usage.count) {
        sufficient.push({
          colorId: stock.color_id, name: stock.name, hex: stock.hex,
          brand: stock.brand, series: stock.series,
          need: usage.count, stock: stock.quantity
        })
      } else if (stock && stock.quantity > 0) {
        insufficient.push({
          colorId: stock.color_id, name: stock.name, hex: stock.hex,
          brand: stock.brand, series: stock.series,
          need: usage.count, stock: stock.quantity, shortage: usage.count - stock.quantity
        })
      } else {
        missing.push({
          hex: usage.hex, need: usage.count, stock: 0, shortage: usage.count
        })
      }
    }
    ...
    for (const item of allForSub) {
      ...
      const alts = inventory
        ...
        .slice(0, 8)
        .map(a => ({
          colorId: a.color_id, name: a.name, hex: a.hex, brand: a.brand,
          series: a.series, deltaE: a.deltaE, inStock: a.quantity,
    ...
    res.json({
      code: 200,
      data: { matchRate, sufficient, insufficient, missing, maxCopies, totalColors, matchedColors, totalBeads, suggestions }
    })
  } catch (e) {
    res.status(500).json({ code: 500, message: e.message })
  }
...
router.get('/inventory/purchase-suggest', authRequired, (req, res) => {
  try {
    ...
    // 3. 图纸缺色（最近编辑的5张图纸）
    const recentDesigns = db.prepare(`
    ...
    for (const d of recentDesigns) {
      const grid = JSON.parse((db.prepare('SELECT grid_data FROM designs WHERE id=?').get(d.id) || {}).grid_data || '[]')
      const usage = {}
      for (const row of (grid || [])) {
        if (!Array.isArray(row)) continue
        for (const cell of row) {
      ...
      for (const [hex, need] of Object.entries(usage)) {
        ...
        if (stock < need) {
          designShortages.push({
            colorId: color.id, name: color.name, hex: color.hex,
            need, stock, shortage: need - stock,
            designTitle: d.title, designId: d.id
          })
        }
      }
    }
    ...
    // 推荐包装
    const specs = db.prepare('SELECT * FROM package_specs WHERE status=1 ORDER BY size, default_count').all()
    ...
    for (const item of [...lowStock, ...designShortages]) {
      ...
      // 计算建议数量
      const shortage = item.shortage || Math.max(0, (item.min_threshold || 200) - (item.quantity || 0))
      ...
      const matchingSpec = specs
        ...
        .sort((a, b) => Math.abs(a.default_count - suggestQty) - Math.abs(b.default_count - suggestQty))[0]

      recommendations.push({
        colorId: key, name: item.name, hex: item.hex, brand: item.brand,
        currentStock: item.quantity || 0, shortage, suggestQty,
        ...
        estimatedCost: matchingSpec ? matchingSpec.reference_price * Math.max(1, Math.ceil(suggestQty / matchingSpec.default_count)) : null
      })
    }
...
router.get('/inventory/color-detail/:colorId', authRequired, (req, res) => {
  try {
    ...
    // 相关图纸
    const relatedDesigns = db.prepare(`
...
router.get('/inventory/substitute/:colorId', authRequired, (req, res) => {
  try {
    ...
    // 目标颜色
    const target = db.prepare('SELECT id, name, hex, lab_l, lab_a, lab_b FROM bead_colors WHERE id=?').get(colorId)
...
router.post('/inventory/multi-match', authRequired, (req, res) => {
  try {
    ...
    for (const did of designIds) {
      ...
      perDesign.push({ designId: did, title: design.title, colors: Object.keys(du).length, beadCount })
    }
    ...
    // 计算缺口
    let totalNeeded = 0, totalShortage = 0
    const items = []
    for (const [hex, need] of Object.entries(usageMap)) {
      ...
      items.push({
        hex, need, inStock, shortage,
        name: stock?.name || '', colorId: stock?.color_id || null
      })
      totalNeeded += need
    ...
    res.json({
      code: 200,
      data: {
        totalNeeded, totalShortage, totalColors: items.length,
        items, perDesign
      },
...
router.get('/warehouses', authRequired, (req, res) => {
  const list = db.prepare('SELECT * FROM warehouse_info WHERE user_id=? AND status=1 ORDER BY is_default DESC').all(req.user.id)
...
router.post('/warehouses', authRequired, (req, res) => {
  ...
  // 会员限制：普通用户最多1个
  const count = db.prepare('SELECT COUNT(*) as c FROM warehouse_info WHERE user_id=?').get(req.user.id)
  const user = db.prepare('SELECT is_vip FROM users WHERE id=?').get(req.user.id)
  const maxWarehouses = user?.is_vip ? 5 : 1
  if (count.c >= maxWarehouses) return res.status(400).json({ code: 400, message: `最多创建${maxWarehouses}个豆仓` })
  const isDefault = count.c === 0 ? 1 : 0
  const r = db.prepare('INSERT INTO warehouse_info (user_id,name,default_spec,default_brand,is_default) VALUES (?,?,?,?,?)')
...
router.get('/custom-colors', authRequired, (req, res) => {
  const list = db.prepare('SELECT * FROM user_custom_colors WHERE user_id=? ORDER BY created_at DESC').all(req.user.id)
  res.json({ code: 200, data: list })
})

router.post('/custom-colors', authRequired, (req, res) => {
  const { colorName, hex, remark } = req.body || {}
  if (!colorName || !hex) return res.status(400).json({ code: 400, message: '请填写颜色名称和色值' })
  // 计算LAB值
  const h = hex.replace('#', '')
  const r = parseInt(h.substring(0, 2), 16), g = parseInt(h.substring(2, 4), 16), b = parseInt(h.substring(4, 6), 16)
  const lab = isNaN(r) ? { L: 0, a: 0, b: 0 } : rgbToLab(r, g, b)
  const r2 = db.prepare('INSERT INTO user_custom_colors (user_id,color_name,hex,lab_l,lab_a,lab_b,remark) VALUES (?,?,?,?,?,?,?)')
    .run(req.user.id, colorName, hex, Math.round(lab.L * 100) / 100, Math.round(lab.a * 100) / 100, Math.round(lab.b * 100) / 100, remark || '')
...
router.post('/wishlist', authRequired, (req, res) => {
  ...
  db.prepare('INSERT INTO wishlist_items (user_id,color_id,priority,notes) VALUES (?,?,?,?)')
    .run(req.user.id, colorId, parseInt(priority) || 0, notes)
...
router.put('/wishlist/:id', authRequired, (req, res) => {
  ...
  if (priority != null) { fields.push('priority=?'); values.push(parseInt(priority)) }
  ...
  db.prepare(`UPDATE wishlist_items SET ${fields.join(',')} WHERE id=? AND user_id=?`).run(...values)
...
router.post('/find-color', (req, res) => {
  try {
    ...
    if (hex) {
      const h = hex.replace('#', '')
      red = parseInt(h.substring(0, 2), 16); green = parseInt(h.substring(2, 4), 16); blue = parseInt(h.substring(4, 6), 16)
    } else { red = r; green = g; blue = b }
    if (isNaN(red) || isNaN(green) || isNaN(blue)) return res.status(400).json({ code: 400, message: '请输入有效颜色值' })

    const targetLab = rgbToLab(red, green, blue)
    const allColors = db.prepare('SELECT id, name, hex, lab_l, lab_a, lab_b, s.name as series, b.name as brand FROM bead_colors c JOIN bead_series s ON c.series_id=s.id JOIN bead_brands b ON s.brand_id=b.id').all()
    const matches = allColors.map(c => ({
      ...c, deltaE: Math.round(deltaE2000(targetLab, { L: c.lab_l, a: c.lab_a, b: c.lab_b }) * 100) / 100
    })).filter(c => c.deltaE < 20).sort((a, b) => a.deltaE - b.deltaE).slice(0, 10)
...
router.post('/compare-colors', (req, res) => {
  try {
    const { colorIds } = req.body || {}
    if (!colorIds?.length || colorIds.length < 2) return res.status(400).json({ code: 400, message: '请至少选择2个颜色' })
    const colors = db.prepare(`SELECT id, name, hex, lab_l, lab_a, lab_b, s.name as series, b.name as brand FROM bead_colors c JOIN bead_series s ON c.series_id=s.id JOIN bead_brands b ON s.brand_id=b.id WHERE c.id IN (${colorIds.map(() => '?').join(',')})`).all(...colorIds)
    if (colors.length < 2) return res.status(400).json({ code: 400, message: '颜色未找到' })

    const pairs = []
    for (let i = 0; i < colors.length; i++) {
      for (let j = i + 1; j < colors.length; j++) {
        const dE = deltaE2000({ L: colors[i].lab_l, a: colors[i].lab_a, b: colors[i].lab_b }, { L: colors[j].lab_l, a: colors[j].lab_a, b: colors[j].lab_b })
        const dL = Math.abs(colors[i].lab_l - colors[j].lab_l)
        const dC = Math.abs(Math.sqrt(colors[i].lab_a ** 2 + colors[i].lab_b ** 2) - Math.sqrt(colors[j].lab_a ** 2 + colors[j].lab_b ** 2))
        pairs.push({
