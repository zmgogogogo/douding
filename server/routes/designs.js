...
router.post('/', authRequired, (req, res) => {
  try {
    const { title, description, gridWidth, gridHeight, gridData, thumbnail, isPublic, brand, folderId } =
      req.body || {}
    ...
    const result = db
      ...
      .run(req.user.id, folderId || null, title, description || '', gridWidth, gridHeight, gridStr,
        thumbnail || null, isPublic ? 1 : 0, brand || 'Hama', beadCount, colorCount)

    const design = db.prepare('SELECT * FROM designs WHERE id = ?').get(result.lastInsertRowid)
    res.json(success(formatDesign(design)))
  } catch (err) {
    console.error('创建设计错误:', err)
    res.status(500).json(fail(500, '创建失败，请稍后重试'))
  }
})
...
router.put('/:id', authRequired, (req, res) => {
  try {
    ...
    if (!design) return res.status(404).json(fail(404, '设计不存在或无权修改'))
    ...
    console.error('更新设计错误:', err)
    res.status(500).json(fail(500, '更新失败，请稍后重试'))
  }
})
...
router.get('/:id', authOptional, (req, res) => {
  try {
    ...
    res.status(500).json(fail(500, '获取失败'))
  }
})
...
router.delete('/:id', authRequired, (req, res) => {
  try {
    const design = db.prepare('SELECT * FROM designs WHERE id = ? AND user_id = ?')
      .get(req.params.id, req.user.id)
    if (!design) return res.status(404).json(fail(404, '设计不存在或无权删除'))
    db.prepare('DELETE FROM designs WHERE id = ?').run(design.id)
    res.json(success(null))
  } catch (err) {
    console.error('删除设计错误:', err)
...
router.get('/', authRequired, (req, res) => {
  try {
    ...
    res.json(paginated(designs.map(formatDesign), total.c, parseInt(page), parseInt(limit)))
  } catch (err) {
