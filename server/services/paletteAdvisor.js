...
function recommendPalette(existingHexes, availableColors, scheme = 'auto') {
  ...
  return {
    scheme,
    schemeLabel: {
      complementary: '互补色', triadic: '三角色',
      analogous: '邻近色', monochromatic: '同色系', auto: '自动'
    }[scheme],
...
function analyzeHarmony(hexColors) {
  ...
  for (let i = 0; i < hsls.length; i++) {
    for (let j = i + 1; j < hsls.length; j++) {
      const li = hsls[i].l, lj = hsls[j].l
      const contrast = Math.abs(li - lj)
      if (contrast < 15) {
        issues.push(`${hexColors[i]} 和 ${hexColors[j]} 亮度太接近 (${contrast.toFixed(0)}%)，拼豆中难以区分`)
        score -= 10
      }
    }
  }
...
function checkColorblindAccessibility(hexColors) {
  ...
  return {
    ...
    summary: allAccessible
      ? '✅ 该配色对所有常见色弱类型友好'
      : `⚠️ 在 ${Object.values(results).filter(r => !r.isAccessible).map(r => r.label).join('、')} 视角下存在难以区分的颜色对`
  }
}
...
function generateFill(options) {
  ...
  switch (type) {
    ...
    case 'gradient':
      ...
      for (let r = 0; r < h; r++) {
        for (let c = 0; c < w; c++) {
          const t = direction === 'v' ? r / Math.max(1, h - 1) :
                     direction === 'h' ? c / Math.max(1, w - 1) :
...
function crossBrandMapping(targetColor, allColors, targetBrand) {
  const sameColor = allColors.filter(c =>
    c.hex.toUpperCase() === targetColor.hex.toUpperCase() &&
    c.brand === targetBrand
  )
  if (sameColor.length) return sameColor[0]
  return findBestMatch(targetColor.hex, allColors.filter(c => c.brand === targetBrand), [], 1)[0] || null
}
...
// ============================================
export {
  hexToRgb, rgbToHsl, colorDistance, deltaE,
  recommendPalette,
  analyzeHarmony,
