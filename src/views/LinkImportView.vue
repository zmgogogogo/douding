  ...
  <div class="fixed inset-0 flex flex-col bg-slate-50 text-slate-900 overflow-hidden select-none z-50">
    ...
    <div class="flex-1 overflow-y-auto">
      <div class="p-4 md:p-8 max-w-2xl mx-auto space-y-6">
        ...
        <!-- 链接输入 -->
        <div class="space-y-3">
          <label class="text-[10px] font-bold uppercase tracking-widest text-slate-400">分享链接</label>
          <div class="flex gap-2">
            <input
              v-model="url"
              type="url"
              placeholder="粘贴小红书分享链接..."
              class="flex-1 h-11 border border-slate-200 rounded-xl px-4 text-sm
                     focus:border-primary focus:ring-1 focus:ring-primary/20 outline-none" />
            <button
              class="h-11 px-6 rounded-xl bg-primary text-white font-bold text-sm
                     hover:bg-primary-dark active:scale-[0.98] transition-all flex items-center gap-2
                     disabled:opacity-40"
        ...
        <!-- 提示 -->
        <div class="flex items-start gap-3 px-4 py-3 bg-amber-50 border border-amber-100 rounded-2xl">
        ...
        <div v-if="parseResult && !loading" class="space-y-4">
          <div class="bg-white rounded-2xl border border-slate-100 p-4 space-y-3">
            <div class="flex items-center gap-2">
              ...
              <span class="font-semibold text-sm text-slate-800">{{ parseResult.title || '解析成功' }}</span>
            </div>
            ...
            <!-- 图片预览 -->
            <div v-if="parseResult.imagePath" class="bg-slate-100 rounded-xl p-3 flex items-center justify-center">
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                ...
                <input v-model.number="targetWidth" type="number" min="10" max="200"
              ...
              <div>
                ...
                <select v-model="brand" class="w-full h-9 border border-slate-200 rounded-lg px-2 text-xs mt-1">
...
import {
  ArrowLeftIcon, SearchIcon, LoaderIcon, CheckCircleIcon,
  AlertCircleIcon, WandIcon
