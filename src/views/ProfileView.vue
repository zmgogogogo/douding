...
<template>
  <div class="overflow-y-auto h-full pb-8">
    ...
    <template v-if="auth.isLoggedIn.value && profile">
      <!-- 用户信息卡片 -->
      <section class="mx-4 mt-4 bg-white rounded-[1.5rem] shadow-sm border border-slate-100/50 overflow-hidden">
        <div class="flex items-center gap-4 px-5 pt-5 pb-4">
          <!-- 头像 -->
          <div class="relative flex-shrink-0 cursor-pointer active:scale-95 transition-transform"
            @click="goEditProfile">
            <div class="w-[60px] h-[60px] rounded-full p-0.5 bg-gradient-to-b from-primary/20 to-transparent">
            ...
            <div class="absolute bottom-0 right-0 w-5 h-5 bg-primary rounded-full flex items-center justify-center
          ...
          <!-- 设置按钮 -->
          <button class="flex-shrink-0 w-9 h-9 rounded-full bg-slate-50 border border-slate-100
        ...
        <div class="flex border-t border-slate-100 mx-1">
          <div class="flex-1 flex flex-col items-center py-3 cursor-pointer
                      active:bg-slate-50/80 transition-colors"
          ...
          <div class="flex-1 flex flex-col items-center py-3 active:bg-slate-50/80 transition-colors">
            <span class="text-[17px] font-bold text-primary leading-tight">
        ...
        <!-- VIP 横幅 -->
        <div class="px-4 pb-4">
          <div class="w-full flex items-center gap-3 rounded-2xl px-3.5 py-3 text-left
                      bg-primary text-white shadow-lg shadow-primary/15
                      active:scale-[0.99] transition-all cursor-pointer"
            @click="toast.show('VIP 功能即将上线')">
            <div class="w-10 h-10 rounded-2xl flex items-center justify-center flex-shrink-0 bg-white/15 text-white">
              <CrownIcon :size="21" />
            </div>
            <div class="flex-1 min-w-0">
      ...
      <section class="px-4 mt-3">
        <div class="bg-white rounded-[1.5rem] overflow-hidden shadow-sm border border-slate-100/50">
          <button v-for="item in serviceMenu" :key="item.label"
            class="w-full flex items-center justify-between px-5 py-[14px]
                   hover:bg-slate-50 transition-colors active:scale-[0.99] duration-150
      ...
      <section class="px-4 mt-3">
        <div class="bg-white rounded-[1.5rem] overflow-hidden shadow-sm border border-slate-100/50">
          <button v-for="item in otherMenu" :key="item.label"
            class="w-full flex items-center justify-between px-5 py-[14px]
                   hover:bg-slate-50 transition-colors active:scale-[0.99] duration-150
                   border-b border-slate-100 last:border-0"
            @click="$router.push(item.route)">
      ...
      <section class="px-4 mt-3 mb-2">
        <div class="bg-white rounded-[1.5rem] overflow-hidden shadow-sm border border-slate-100/50">
          <button
            class="w-full flex items-center justify-between px-5 py-[14px]
                   hover:bg-red-50 transition-colors active:scale-[0.99] duration-150"
            @click="handleLogout">
            <div class="flex items-center gap-3.5">
    ...
    <div v-else-if="!loading" class="flex flex-col items-center py-16 px-6">
      ...
      <button class="mt-6 px-10 py-3 bg-primary text-white rounded-full font-bold text-sm
...
import {
  UserIcon, PencilIcon, SettingsIcon, CrownIcon, ChevronRightIcon,
  HeartIcon, PackageIcon, BookIcon, ClockIcon, MessageCircleIcon,
  InfoIcon, LogOutIcon, LogInIcon
...
const serviceMenu = [
  { label: '我的喜欢', icon: HeartIcon, color: 'text-red-400', bgColor: 'bg-red-50', route: '/likes' },
  { label: '豆子仓库', icon: PackageIcon, color: 'text-violet-500', bgColor: 'bg-violet-50', route: '/warehouse',
    badge: 'NEW', badgeClass: 'bg-violet-500 text-white' },
]

const otherMenu = [
  { label: '使用教程', icon: BookIcon, color: 'text-amber-500', bgColor: 'bg-amber-50', route: '/tutorial' },
  ...
  { label: '意见反馈', icon: MessageCircleIcon, color: 'text-blue-400', bgColor: 'bg-blue-50', route: '/feedback',
    badge: '💬 说说你的想法', badgeClass: 'bg-orange-50 text-orange-500 border border-orange-200' },
  { label: '关于我们', icon: InfoIcon, color: 'text-slate-500', bgColor: 'bg-slate-100', route: '/about' },
]

const isOwner = computed(() => auth.user.value && profile.value &&
  auth.user.value.id === profile.value.id)

onMounted(async () => {
  if (!auth.isLoggedIn.value) { loading.value = false; return }
...
async function goEditProfile() {
  const nickname = await dialog.prompt('请输入新昵称', '编辑资料', profile.value?.nickname || '', '昵称')
  ...
  if (bio !== null) {
    API.put('/api/auth/profile', { nickname, bio }).then(() => {
      ...
      if (profile.value) { profile.value.nickname = nickname; profile.value.bio = bio }
      toast.show('资料已更新')
