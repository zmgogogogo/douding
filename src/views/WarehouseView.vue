...
<template>
  <div class="flex flex-col h-full" v-if="auth.isLoggedIn.value">
    <!-- Tab 切换 -->
    <div
      class="flex items-center gap-1 px-4 pt-3 pb-2 border-b border-slate-100 bg-white flex-shrink-0"
    >
      <button
        v-for="t in tabs"
        :key="t.key"
    ...
    <div v-if="activeTab === 'inventory'" class="flex-1 overflow-y-auto p-4">
      <div
        id="scan-reader"
        v-show="scanning"
        class="max-w-sm mx-auto mb-3 rounded-xl overflow-hidden"
      />
      <!-- 扫码结果反馈 -->
      <div
        v-if="scanResult"
        class="max-w-sm mx-auto mb-3 px-3 py-2 rounded-lg text-xs text-center font-medium"
        :class="
          scanResult.includes('已入库')
    ...
    <div v-if="activeTab === 'designs'" class="flex flex-1 overflow-hidden">
      ...
      >
        <h3 class="text-[11px] uppercase tracking-wide text-slate-400 mb-2 font-semibold">
          📁 文件夹
        </h3>
        ...
        <button
          class="w-full mt-2 h-8 text-[11px] text-slate-500 rounded-lg hover:bg-slate-50 transition-colors"
          @click="newFolder"
        >
          + 新建文件夹
        </button>
      ...
      <div v-if="purchaseLists.length" class="space-y-4">
        ...
        >
          <div class="flex items-center justify-between mb-2">
            ...
            >
              {{
                { draft: '草稿', ordered: '已下单', arrived: '已到货', cancelled: '已取消' }[
                  list.status
                ]
              }}</span
            >
          ...
          <div class="space-y-1">
            ...
            >
              ...
              <span v-if="item.purchased_quantity" class="text-emerald-500"
                >已购{{ item.purchased_quantity }}</span
              >
            </div>
          </div>
        </div>
    ...
    <div v-if="activeTab === 'stats'" class="flex-1 overflow-y-auto p-4">
      ...
      <div class="grid grid-cols-4 gap-3 mb-6">
        <div class="stat-card">
          <div class="stat-num">{{ statsData?.overview?.tracked_colors || 0 }}</div>
          <div class="stat-label">追踪颜色</div>
        </div>
        <div class="stat-card">
          <div class="stat-num">
            {{ (statsData?.overview?.total_in_stock || 0).toLocaleString() }}
          </div>
          <div class="stat-label">总库存（颗）</div>
        </div>
        <div class="stat-card">
          <div class="stat-num text-amber-500">
            {{ statsData?.overview?.total_in_transit || 0 }}
          </div>
          <div class="stat-label">运输中</div>
        </div>
        <div class="stat-card">
          <div class="stat-num text-blue-500">{{ alertCount }}</div>
          <div class="stat-label">需补豆</div>
        </div>
      </div>
      <StatsCharts />
    </div>
    ...
    await html5QrCode.start(
      ...
      async (decodedText) => {
        ...
        if (found) {
          await API.post(
            '/api/inventory',
            { colorId: found.id, quantity: 100, note: '扫码入库' },
            true
          )
...
.stat-label {
  @apply text-[10px] text-slate-400 mt-1;
}
</style>
