...
<template>
  <div class="overflow-y-auto h-full scrollbar-hide">
    ...
    <div class="flex items-center gap-3 px-4 pt-4 pb-2 sticky top-0 bg-[#f8fafc]/80 backdrop-blur-sm z-10">
      ...
      <h1 class="text-lg font-bold text-slate-900">我的收藏</h1>
    </div>
    ...
    <div v-else-if="!items.length" class="flex flex-col items-center py-20 text-slate-400">
      <BookmarkIcon :size="48" class="text-slate-200 mb-3" />
      <p class="text-sm">还没有收藏过作品</p>
      <button
        ...
        >
          ...
          <div style="padding:10px">
            ...
            <div style="display:flex;gap:12px;font-size:11px;color:#94a3b8">
              <button
                style="display:flex;align-items:center;gap:2px;border:none;background:none;cursor:pointer;padding:0;font:inherit;color:#f59e0b"
                @click.stop="handleUnfavorite(item)"
              >
                <BookmarkIcon :size="12" class="fill-amber-500 text-amber-500" />
...
import { LoaderIcon, ChevronLeftIcon, BookmarkIcon } from 'lucide-vue-next'
...
async function fetchData(reset = false) {
  ...
  try {
    ...
    const res = await API.get(`/api/user/favorites?${params}`)
...
async function handleUnfavorite(item) {
  try {
    const res = await API.post(`/api/designs/${item.id}/favorite`)
    if (res.code === 200 && !res.data.favorited) {
      // 取消收藏成功，从列表中移除
