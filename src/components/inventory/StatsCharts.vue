      ...
      <div class="flex items-center justify-between mb-3">
        ...
        <div class="flex gap-1">
          <button v-for="p in periods" :key="p.key"
            ...
            :class="period === p.key ? 'bg-primary/10 text-primary' : 'text-slate-400 hover:bg-slate-50'"
            @click="period = p.key; loadTrend()">{{ p.label }}</button>
        </div>
      </div>
...
let trendInstance = null, rankInstance = null, pieInstance = null
  ...
  try {
    ...
    if (trendInstance && byColor.length > 0) {
      ...
      trendInstance.setOption({
        ...
        xAxis: { type: 'category', data: trendData.map(d => d.date), axisLabel: { fontSize: 9 } },
        yAxis: { type: 'value', splitLine: { lineStyle: { color: '#e2e8f0' } }, axisLabel: { fontSize: 9 } },
        series: [
          { name: '总消耗', type: 'line', data: trendData.map(d => d.inbound), smooth: true, lineStyle: { color: '#3b82f6' }, symbol: 'none' },
          { name: '出库', type: 'line', data: trendData.map(d => d.outbound), smooth: true, lineStyle: { color: '#ef4444' }, symbol: 'none' }
        ]
      }, true)
    }
    ...
    if (rankInstance && byColor.length > 0) {
      ...
      rankInstance.setOption({
        ...
        xAxis: { type: 'value', splitLine: { lineStyle: { color: '#e2e8f0' } }, axisLabel: { fontSize: 9 } },
        yAxis: { type: 'category', data: top15.map(c => c.name?.length > 6 ? c.name.slice(0,6)+'…' : c.name), axisLabel: { fontSize: 9 } },
        series: [{
          type: 'bar', data: top15.map((c, i) => ({ value: c.total_used, itemStyle: { color: c.hex || darkColors[i % darkColors.length] } })),
          barWidth: 10, itemStyle: { borderRadius: [0, 3, 3, 0] }
        }]
      }, true)
    }
