  ...
  <Teleport to="body">
    <Transition name="dialog">
      <div v-if="visible" class="fixed inset-0 z-[160] flex items-center justify-center bg-black/30 backdrop-blur-sm"
        ...
        <div class="bg-white rounded-2xl shadow-xl w-[480px] max-w-[94vw] max-h-[85vh] flex flex-col overflow-hidden animate-bounce-in">
          <!-- 头部 -->
              ...
              <div class="relative w-16 h-16 flex-shrink-0">
                <svg class="w-full h-full -rotate-90" viewBox="0 0 36 36">
                  ...
                  <circle cx="18" cy="18" r="15.5" fill="none"
                    :stroke="matchColor" stroke-width="3" stroke-linecap="round"
                    :stroke-dasharray="Math.PI * 31"
                    :stroke-dashoffset="Math.PI * 31 * (1 - matchData.matchRate / 100)" />
                </svg>
                <div class="absolute inset-0 flex items-center justify-center">
                  <span class="text-sm font-bold" :class="matchTextColor">{{ matchData.matchRate }}%</span>
                </div>
              </div>

              <!-- 分类标签 -->
              <div class="flex flex-wrap gap-1.5">
                <span class="px-2 py-0.5 rounded-full text-[10px] font-medium bg-green-50 text-green-600">
                  🟢 充足 {{ matchData.sufficient?.length || 0 }}色
                </span>
                <span class="px-2 py-0.5 rounded-full text-[10px] font-medium bg-amber-50 text-amber-600">
                  🟡 不足 {{ matchData.insufficient?.length || 0 }}色
                </span>
                <span class="px-2 py-0.5 rounded-full text-[10px] font-medium bg-red-50 text-red-500">
                  🔴 缺失 {{ matchData.missing?.length || 0 }}色
                </span>
                <span class="px-2 py-0.5 rounded-full text-[10px] font-medium bg-slate-50 text-slate-500">
          ...
          <div class="flex-1 overflow-y-auto px-5 pb-3">
            ...
            <template v-if="matchData.missing?.length">
              ...
              <div v-for="item in matchData.missing" :key="item.hex"
                class="flex items-center gap-2 py-1.5 border-l-2 border-red-400 pl-2 mb-1 bg-red-50/30 rounded-r-lg">
                <div class="w-4 h-4 rounded-sm ring-1 ring-black/10 flex-shrink-0" :style="{ background: item.hex }" />
                <span class="text-[11px] text-slate-600 flex-1 truncate">{{ item.name || item.hex }}</span>
                <span class="text-[11px] text-slate-400 font-mono">需{{ item.need }}</span>
                <span class="text-[11px] text-red-500 font-mono font-bold">缺{{ item.shortage }}</span>
                <button v-if="matchData.suggestions?.[item.hex]?.length"
                  class="text-[10px] text-primary hover:underline flex-shrink-0"
                  @click.stop="toggleSub(item.hex)">替代色</button>
              </div>
            </template>
            ...
            <template v-if="matchData.insufficient?.length">
              ...
              <div v-for="item in matchData.insufficient" :key="item.hex"
                ...
                <span class="text-[11px] text-amber-600 font-mono font-bold">缺{{ item.shortage }}</span>
                <button v-if="matchData.suggestions?.[item.hex]?.length"
                  class="text-[10px] text-primary hover:underline flex-shrink-0"
                  @click.stop="toggleSub(item.hex)">替代色</button>
              </div>
            ...
            <template v-if="matchData.sufficient?.length">
              ...
              <div v-for="item in matchData.sufficient" :key="item.hex"
            ...
            <!-- 替代色展开区域 -->
            <div v-if="activeSubHex && matchData.suggestions?.[activeSubHex]?.length"
              class="bg-blue-50/30 rounded-lg p-2.5 mb-2">
              <div class="text-[10px] font-semibold text-slate-600 mb-1.5">
                {{ activeSubHex }} 的替代色推荐
              </div>
              <div class="space-y-1">
                <div v-for="alt in matchData.suggestions[activeSubHex]" :key="alt.colorId"
                  class="flex items-center gap-2 text-[10px]">
                  <div class="w-4 h-4 rounded-sm ring-1 ring-black/10 flex-shrink-0" :style="{ background: alt.hex }" />
                  <span class="text-slate-600 flex-1">{{ alt.name }}</span>
                  <span class="font-mono" :class="alt.grade === 'excellent' ? 'text-green-500' : alt.grade === 'good' ? 'text-amber-500' : 'text-slate-400'">
                    ΔE{{ alt.deltaE }}
                  </span>
          ...
          <div class="flex border-t border-slate-100 flex-shrink-0">
            <button class="flex-1 h-10 text-xs text-primary font-medium hover:bg-primary/5 transition-colors"
              @click="addAllToPurchase">📋 全选加入采购清单</button>
