...
<template>
  <Teleport to="body">
    <Transition name="dialog">
      <div v-if="visible" class="fixed inset-0 z-[160] flex items-center justify-center bg-black/30 backdrop-blur-sm"
        @click.self="$emit('close')">
        <div class="bg-white rounded-2xl shadow-xl w-[420px] max-w-[92vw] max-h-[85vh] flex flex-col overflow-hidden animate-bounce-in">
          <!-- 头部 -->
          <div class="flex items-center justify-between px-5 pt-5 pb-3 flex-shrink-0">
            ...
            <button class="w-7 h-7 rounded-full hover:bg-slate-100 flex items-center justify-center transition-colors"
              @click="$emit('close')">
          ...
          <template v-else-if="detail">
            <div class="flex-1 overflow-y-auto px-5 pb-5 space-y-4">
              ...
              <div class="flex gap-4">
                <div class="w-24 h-24 rounded-xl ring-1 ring-black/10 flex-shrink-0"
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div v-if="detail.color?.lab_l" class="text-[10px] text-slate-400 mt-0.5">
                    LAB {{ detail.color.lab_l?.toFixed(1) }}, {{ detail.color.lab_a?.toFixed(1) }}, {{ detail.color.lab_b?.toFixed(1) }}
                  </div>
                </div>
              </div>
              ...
              <div class="bg-slate-50 rounded-xl p-3 grid grid-cols-3 gap-2 text-center">
                <div>
                  <div class="text-lg font-bold" :class="invQty === 0 ? 'text-red-500' : 'text-slate-800'">
                    {{ (invQty || 0).toLocaleString() }}
                  </div>
                  <div class="text-[10px] text-slate-400">当前库存</div>
                </div>
                <div>
                  <div class="text-lg font-bold text-amber-500">{{ (detail.inventory?.transitQuantity || 0).toLocaleString() }}</div>
                  <div class="text-[10px] text-slate-400">运输中</div>
                </div>
                <div>
                  <div class="text-lg font-bold text-slate-600">{{ (detail.inventory?.minThreshold || 0).toLocaleString() }}</div>
              ...
              <div class="flex items-center gap-2">
                ...
                <span class="text-xs font-mono font-bold text-slate-700 w-14 text-center">{{ adjustQty }}</span>
                <button class="quick-btn" @click="quickAdjust(10)">+10</button>
                <button class="quick-btn" @click="quickAdjust(100)">+100</button>
                <button class="h-7 px-2 rounded-lg text-[10px] font-medium bg-primary text-white hover:bg-primary-dark transition-colors"
                  @click="doAdjust" :disabled="adjustQty === (detail.inventory?.quantity || 0)">确定</button>
              </div>
              ...
              <div v-if="detail.alternatives?.length">
                ...
                <div class="flex gap-2 overflow-x-auto pb-1">
                  <div v-for="alt in detail.alternatives" :key="alt.colorId"
                    class="flex-shrink-0 w-[72px] bg-slate-50 rounded-lg p-1.5 text-center cursor-pointer
                           hover:bg-slate-100 transition-colors"
                    @click="$emit('select-substitute', alt)">
                    <div class="w-8 h-8 rounded-md ring-1 ring-black/10 mx-auto mb-1"
                      :style="{ background: alt.hex }" />
                    <div class="text-[9px] text-slate-600 truncate">{{ alt.name }}</div>
                    <div class="text-[9px] font-mono"
                      :class="alt.grade === 'excellent' ? 'text-green-500' : alt.grade === 'good' ? 'text-amber-500' : 'text-slate-400'">
                      ΔE{{ alt.deltaE }}
                    </div>
              ...
              <div v-if="detail.logs?.length">
                ...
                <div class="space-y-1.5 max-h-[200px] overflow-y-auto">
                  <div v-for="log in displayLogs" :key="log.id"
                    ...
                    <span class="text-slate-400 w-14 flex-shrink-0">{{ formatDate(log.created_at) }}</span>
                    <span :class="log.quantity > 0 ? 'text-green-600' : 'text-red-500'" class="font-mono">
                      {{ log.quantity > 0 ? '+' : '' }}{{ log.quantity }}
                    </span>
                    <span class="text-slate-500 truncate flex-1">{{ log.source_name || log.note || log.source_type || '' }}</span>
                  </div>
                </div>
                <button v-if="detail.logs.length > 20" class="text-[10px] text-primary mt-1 hover:underline"
              ...
              <div v-if="detail.relatedDesigns?.length">
                ...
                <div class="flex gap-2 overflow-x-auto">
                  <div v-for="d in detail.relatedDesigns" :key="d.id"
            ...
            <div class="flex border-t border-slate-100 flex-shrink-0">
              <button class="flex-1 h-10 text-xs text-slate-500 font-medium hover:bg-slate-50 transition-colors"
                @click="addToPurchaseList">📋 加入采购清单</button>
              <div class="w-px bg-slate-100" />
              <button class="flex-1 h-10 text-xs text-red-400 font-medium hover:bg-red-50 transition-colors"
...
const invQty = computed(() => detail.value?.inventory?.quantity ?? props.inventoryItem?.quantity ?? 0)
const displayLogs = computed(() => {
  const logs = detail.value?.logs || []
  return showAllLogs.value ? logs : logs.slice(0, 20)
})

watch(() => [props.colorId, props.visible], async ([id, vis]) => {
  if (!id || !vis) return
  loading.value = true
...
async function addToPurchaseList() {
  try {
    const res = await API.post('/api/inventory/purchase-list', {
