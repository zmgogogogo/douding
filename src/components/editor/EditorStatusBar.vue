<!-- ============================================
  ...
  ============================================ -->
<template>
  <footer class="h-7 bg-[var(--ui-bg-surface)] border-t border-[var(--ui-border)] flex items-center px-3 gap-4 select-none flex-shrink-0 text-[10px] z-10">
    <!-- 左侧：模式 + 尺寸 + 豆子数 + 颜色数 -->
    <div class="flex items-center gap-3">
      ...
      <span class="text-[var(--ui-text-tertiary)]">
        <span class="text-[var(--ui-text-secondary)] font-medium">{{ gridW }}×{{ gridH }}</span> 豆子
      </span>
      <span class="text-[var(--ui-text-tertiary)]">
    ...
    <div class="flex-1 flex items-center justify-center gap-2">
      <span class="text-[var(--ui-text-tertiary)] font-mono">
        X: <span class="text-[var(--ui-text-secondary)]">{{ mouseCol >= 0 ? mouseCol : '–' }}</span>
        &nbsp;
        Y: <span class="text-[var(--ui-text-secondary)]">{{ mouseRow >= 0 ? mouseRow : '–' }}</span>
      </span>
      <template v-if="mouseColor">
        <span class="w-px h-3 bg-[var(--ui-border)]" />
        <span class="w-3 h-3 rounded-sm ring-1 ring-black/10 flex-shrink-0" :style="{ background: mouseColor.hex }" />
    ...
    <div class="flex items-center gap-1.5">
      <button class="w-5 h-5 rounded flex items-center justify-center hover:bg-[var(--ui-bg-tertiary)] text-[var(--ui-text-secondary)] transition-colors"
        @click="$emit('zoomOut')" title="缩小">−</button>
      <input
        type="range"
      ...
      />
      <button class="w-5 h-5 rounded flex items-center justify-center hover:bg-[var(--ui-bg-tertiary)] text-[var(--ui-text-secondary)] transition-colors"
        @click="$emit('zoomIn')" title="放大">+</button>
      <span class="text-[var(--ui-text-secondary)] font-mono font-medium w-9 text-right tabular-nums">
        {{ Math.round(zoom * 10) }}%
      </span>
    </div>
  </footer>
