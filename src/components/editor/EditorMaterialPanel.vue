  ...
  <div class="flex-1 flex flex-col overflow-hidden text-[11px]">
    ...
    <div class="px-3 py-2 border-b border-[var(--ui-border)] bg-[var(--ui-bg-tertiary)]/50">
      <div class="grid grid-cols-2 gap-1 text-[10px]">
        <div class="flex justify-between">
          ...
          <span class="font-semibold">{{ totalBeads }} 颗</span>
        </div>
        <div class="flex justify-between">
          <span class="text-[var(--ui-text-tertiary)]">总重量</span>
          <span class="font-semibold">{{ totalWeight.toFixed(1) }}g</span>
        </div>
        <div class="flex justify-between">
          <span class="text-[var(--ui-text-tertiary)]">预估时长</span>
          <span class="font-semibold">{{ estimatedTime }}</span>
        </div>
        <div class="flex justify-between">
          <span class="text-[var(--ui-text-tertiary)]">预估成本</span>
          <span class="font-semibold text-[var(--ui-accent)]">¥{{ totalCost.toFixed(2) }}</span>
        </div>
      </div>
    </div>
    ...
    <div class="flex-1 overflow-y-auto">
      ...
      >
        ...
        <span class="font-mono text-[var(--ui-text-secondary)] w-10 text-right"
          >{{ (s.count * gramPerBead).toFixed(1) }}g</span
        >
      </div>
    </div>
    ...
    <div class="flex gap-1 px-2 py-1.5 border-t border-[var(--ui-border)]">
      <button
        class="flex-1 h-6 rounded text-[10px] bg-[var(--ui-bg-tertiary)] hover:bg-[var(--ui-accent)]/10 hover:text-[var(--ui-accent)] transition-colors"
        @click="$emit('exportCSV')"
      >
        📋 导出 CSV
      </button>
      <button
        class="flex-1 h-6 rounded text-[10px] bg-[var(--ui-accent)]/10 text-[var(--ui-accent)] hover:bg-[var(--ui-accent)]/20 transition-colors"
