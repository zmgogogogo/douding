  ...
  <div class="flex-1 flex flex-col overflow-hidden text-[11px]">
    ...
    <div class="px-3 py-2 border-b border-[var(--ui-border)]">
      <div class="flex items-center gap-2">
        ...
        <button
          v-if="!connected"
          class="ml-auto text-[10px] text-[var(--ui-accent)] hover:underline"
          @click="$emit('connect')"
        >
          连接
        </button>
    ...
    <div class="flex-1 overflow-y-auto">
      <div
        v-for="m in members"
        :key="m.socketId"
        class="flex items-center gap-2 px-3 py-1.5 border-b border-[var(--ui-border)]/50"
      >
        <div
      ...
      <div
        v-if="!members.length"
        class="text-[10px] text-[var(--ui-text-tertiary)] text-center py-6"
      >
    ...
    <div class="px-3 py-2 border-t border-[var(--ui-border)] space-y-1.5">
      ...
      <div class="flex gap-1">
        ...
        />
        <button
          class="px-2 h-6 rounded text-[10px] bg-[var(--ui-accent)] text-white hover:opacity-90"
          @click="copyLink"
        >
          {{ copied ? '✓' : '复制' }}
        </button>
      ...
      <div class="flex items-center gap-2 text-[9px] text-[var(--ui-text-tertiary)]">
        <select
          v-model="permission"
          class="border rounded px-1 py-0.5 text-[9px]"
          @change="$emit('setPermission', permission)"
        >
    ...
    <div v-if="comments.length" class="border-t border-[var(--ui-border)] max-h-32 overflow-y-auto">
      <div
        v-for="c in comments"
        :key="c.id"
        class="px-2 py-1 border-b border-[var(--ui-border)]/50 text-[10px]"
        :class="{ 'opacity-50': c.resolved }"
      >
        <div class="flex items-center gap-1">
          <span class="font-medium">{{ c.nickname }}</span>
          <span v-if="c.r !== undefined" class="text-[var(--ui-text-tertiary)]"
            >({{ c.r }},{{ c.c }})</span
          >
          <button
            v-if="!c.resolved"
            class="ml-auto text-[8px] text-[var(--ui-accent)]"
            @click="$emit('resolveComment', c.id)"
          >
            解决
          </button>
        </div>
