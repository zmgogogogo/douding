...
<template>
  <div
    class="absolute top-3 left-1/2 -translate-x-1/2 glass-panel rounded-2xl px-3 py-1.5 flex items-center gap-2 z-10 animate-scale-in select-none"
    style="box-shadow: var(--ui-shadow-lg)"
  >
    <!-- 上一个 -->
    <button
      class="w-5 h-5 rounded-full bg-[var(--ui-bg-tertiary)] flex items-center justify-center hover:bg-[var(--ui-border)] transition-colors disabled:opacity-30"
      @click="$emit('prev')"
      :disabled="!hasPrev"
    >
    ...
    <div class="flex items-center gap-1.5">
      ...
      <span class="text-[9px] text-[var(--ui-text-tertiary)]"
        >{{ currentColor?.count || 0 }}颗</span
      >
    </div>

    <!-- 下一个 -->
    <button
      class="w-5 h-5 rounded-full bg-[var(--ui-bg-tertiary)] flex items-center justify-center hover:bg-[var(--ui-border)] transition-colors"
      @click="$emit('next')"
    >
    ...
    <button
      ...
      :class="
        autoPlay
          ...
          : 'bg-[var(--ui-bg-tertiary)] text-[var(--ui-text-tertiary)]'
      "
    ...
    <!-- 退出 -->
    <button
      class="text-[9px] text-[var(--ui-text-tertiary)] hover:text-red-500 transition-colors"
      @click="$emit('exit')"
    >
      ✕
    </button>
