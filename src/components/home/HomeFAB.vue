...
<template>
  ...
  <Transition name="slide-up">
    <div
      v-if="menuOpen"
      class="fixed bottom-24 right-4 z-50 bg-white rounded-2xl shadow-xl border border-slate-100
             overflow-hidden w-48"
    >
      <button
        v-for="opt in createOptions" :key="opt.label"
        class="w-full flex items-center gap-3 px-4 py-3 text-sm text-slate-700
               hover:bg-slate-50 active:bg-slate-100 transition-colors border-b border-slate-50
               last:border-b-0"
        @click="onSelect(opt)"
      >
  ...
  <Transition name="fab-fade">
    <button
      v-show="visible"
      class="fixed bottom-20 right-4 z-50 w-14 h-14 rounded-full
